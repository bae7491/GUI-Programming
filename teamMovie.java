...
public class teamMovie {
	...
	void viewToModel() {
		m = new MovieModel(v.ttf.getText(), v.tf[0].getText(), v.combG.getSelectedItem().toString(), v.combT.getSelectedItem().toString(), v.tf[1].getText(), v.combR.getSelectedItem().toString(), v.combC.getSelectedItem().toString());
	}
	
	void addTextArea() {
		String s;
		s = String.format("%-15s %-8s %-20s %-30s %10s %10s %2s\n", m.title, m.director, m.genre, m.time, m.actor, m.rating, m.country);
		v.ta.append(s);
	}
	...
	public void addMovie() {
		...
		try {
			...
			s += "('" + m.title + "', '" + m.director + "', '" + m.genre + "', '" + m.time + "', '" + m.actor + "', '" + m.rating + "', '" + m.country + "')";
